...
# ╔═╡ 18d9e080-ac17-11ec-1845-897491a08319
begin
	import CairoMakie
	using Flux, FastAI, DataFramesMeta, RData
end

# ╔═╡ 41234cee-7c8d-4ce9-a1fa-8bdaf1de2f74
using RCall, MLJ

# ╔═╡ 841da28c-b48e-4bb4-8c36-9fd09427d7ce
using Dates, Colors, StatsPlots

# ╔═╡ aac22bd4-1435-4c75-a59f-0d4e93f42c56
using Interact, PlutoUI

# ╔═╡ f4e60282-37af-4ee7-a2ec-836e263812da
CairoMakie.activate!(type = "png")

# ╔═╡ 0825feb0-2dea-4649-88c2-5cb17802da5a
import Plots

# ╔═╡ ea0d4501-5007-4fe9-9a16-d37edc64bbd3
TablesofContent

# ╔═╡ f20d1c83-0c79-4db0-ab09-71f1da9597d3
df = rcopy(R"""readRDS("N:/ATHE/HELG/Office/Technik/03_Fachbereiche/Prozessentwicklung/02_Datentransparenz/datenlesen/data_ZF/dfFokus.rds")""")

# ╔═╡ f2cb1772-0f21-4cd8-978a-da4df2c430ba
dat = @chain df begin
	@rsubset :Bauteil == "HP09120"
	#@rsubset :ANFA == "0"
	@rsubset !ismissing(:BA)
	dropmissing([:M1, :M2, :TZ, :SchussNr, :CATEGORY])
end

# ╔═╡ 3d5b125e-a80a-4729-92c0-f54b6c1c8ba4
dat[!, :CATEGORY_reduced ] .= ifelse.(dat.CATEGORY .== "Gut", "iO", "NiO")

# ╔═╡ fbefd95f-b0ab-4da1-acb9-89634ed8d3c0
y, X_indentifier, X_cat, X_cont, rest = unpack(dat, ==(:CATEGORY), in([:FormNr, :Bauteil, :MACHINED_PART]), in(Symbol.(["DGM", "BA", "MACHINING_PLANT_LINE"])), in(Symbol.(["DR", "M1", "M2", "H3", "TZ", "V1", "V2"])))

# ╔═╡ b8beb299-e3f3-4488-b4bb-7a24c6c49919
begin
	param = "DR"
	plots = []
	for (ix,form) in enumerate(unique(dat.FormNr))
		dat2 = @rsubset(dat, :FormNr == form)
		param_val = dat2[!,param]

		p1 = Plots.scatter(
			dat2.Datum, param_val, group = dat2.DGM,
			msw = 0.1, title = form,
			legendtitle = "DGM"
			#color = convert(Array{String}, replace(dat2.ANFA,"0" => "steelblue", "1" => "red", missing => "orange"))
		)

		p2 = Plots.scatter(
			dat2.Datum, param_val, group = dat2.ANFA,
			msw = 0.1, title = form,
			color = convert(Array{String}, replace(dat2.ANFA,"0" => "steelblue", "1" => "red", missing => "orange")),
			legendtitle = "ANFA"
		)
		...
		p3 = Plots.scatter(
			dat2.Datum, param_val, group = dat2.CATEGORY_reduced,
			msw = 0.1, title = form,
			color = convert(Array{String}, replace(dat2.CATEGORY_reduced,"iO" => "steelblue", "NiO" => "orange")),
			legendtitle = "CATEGORY",
				ms = 1
		)
		push!(
			plots, Plots.plot(p1,p2, p3, layout = (1,3), link = :all)
		)
	end
	p = Plots.plot(plots..., layout = (length(unique(dat.FormNr)),1), legend = :outertopright, link = :all, plot_title = "HP09120",plot_xlab = "Datum", ylab = "$param", size = [2000,2000])

	#savefig(p, "../results/HP09120_analysis_$param.png")
end

# ╔═╡ 2d50d232-40f2-47a8-8126-579fe33771e9
Plots.gr()

# ╔═╡ 7bbb6ebc-238e-4819-98d8-defd566ebc07
md"""
# FastAI Modeling
"""

# ╔═╡ 54cf6e38-2557-4070-8685-b0ff959dc927
data = TableDataset(df)

# ╔═╡ c27b9326-18db-4659-aa39-b57dc4d9ae6c
begin
...
git-tree-sha1 = "0b95c5bb61190bfb60b8e3f06c5e950a80168cf7"
...
[[FastAI]]
deps = ["Animations", "BSON", "CSV", "Colors", "DataAugmentation", "DataDeps", "DataFrames", "DataLoaders", "FileIO", "FilePathsBase", "FixedPointNumbers", "Flux", "FluxTraining", "Glob", "ImageInTerminal", "IndirectArrays", "InlineTest", "JLD2", "LearnBase", "MLDataPattern", "Markdown", "MosaicViews", "Parameters", "PrettyTables", "Reexport", "Requires", "Setfield", "ShowCases", "StaticArrays", "Statistics", "Tables", "Test", "UnicodePlots", "Zygote"]
git-tree-sha1 = "b1fa7b140daffcdb73cbc93db44423c1ee78961c"
...
[[FluxTraining]]
deps = ["Animations", "BSON", "DataStructures", "EarlyStopping", "Flux", "Glob", "Graphs", "ImageCore", "InlineTest", "OnlineStats", "Parameters", "PrettyTables", "ProgressMeter", "Reexport", "Statistics", "TensorBoardLogger", "UUIDs", "ValueHistories", "Zygote"]
git-tree-sha1 = "42ab0fdaf31c0f01a5e4a0cb7cbd6dcc9203475d"
...
[[ImageInTerminal]]
deps = ["Crayons", "ImageCore", "ImageTransformations"]
git-tree-sha1 = "835047c78830901a1b70c73e2e50ec66fb8a80d6"
...
git-tree-sha1 = "9e7609c29736460e258bacfc5c35c3d31bfa90dd"
